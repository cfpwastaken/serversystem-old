...
module.exports = {
    ...
    callback: async function(message, arguments, text) {
        ...
        if(message.channel.name !== serverstats[message.guild.id].ticket) return message.reply("Du kannst in diesem Channel kein Ticket erstellen!");
        
        message.delete();

        let category = message.guild.channels.cache.find(ct=>ct.name === serverstats[message.guild.id].ticketcat && ct.type === "category");

        if(!category) await message.guild.channels.create("tickets", {type: "category"}).then(cat=>category = cat);

        if(message.guild.channels.cache.find(cha=>cha.name===`ticket-${username.toLowerCase()}`)) return message.reply("Ein Ticket von dir läuft bereits");

        let supporter = message.guild.roles.cache.find(rl=>rl.name=== serverstats[message.guild.id].support);

        if(!supporter) return message.reply("Eine Supporter rolle konnte nicht gefunden werden, stelle sicher das die Supporterrolle eingerichtet ist!");
        ...
        await message.guild.channels.create(`ticket-${message.author.username}`, {type:"text"}).then(ch=>{
            ...
            ch.send(`<@&${supporter.id}>`, embed=require("../../utils/util-functions.js").embed("Ticket", "Beschreibe dein Problem hier.", "RANDOM", "TicketSystem"));
        }).catch(err=>{
