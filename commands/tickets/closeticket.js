...
module.exports = {
    ...
    callback: async function(message, arguments, text) {
        ...
        fetchTranscript(message, 99)
            .then((data) => {
                const file = new discord.MessageAttachment(data, "transcript-" + username.toLowerCase() + "-" + Math.floor(Math.random() * (9999 - 1000) + 1000) + ".html");
                const channel = message.guild.channels.cache.find(ch => ch.name === serverstats[message.guild.id].transcript);
                // const channel = message.guild.channels.cache.find(ch => ch.name === "transcript");
                channel.send(file);
            });
